...
namespace Game
{
    ...
    class Healer: Unit
    {
        ...
        public Healer(int hp, int damage, int stamina, int armor,string name,HealingDevice dev, Appearance ap) :base(hp,damage,stamina,armor,name, ap)
        {
            hDev = dev;
            SetHealCap();
        }

        public Healer(Healer ob,HealingDevice device):base(ob)
        {
            hDev = device;
            SetHealCap();
        }
